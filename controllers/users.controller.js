...
const create = async(req, res) =>{
    ...
    //confirm data
    if(!email || !password || !confirm_password) return res.status(400).json({ message: 'Email, password and confirm password Fields are required' })
...
const update = async(req, res) =>{
    ...
    //confirm data
    if(!id || !email || !Array.isArray(roles) || !roles.length || typeof active !== 'boolean') return res.status(400).json({ message: 'Email and ID Fields are required' })
