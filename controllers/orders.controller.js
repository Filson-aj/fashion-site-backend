...
const index = async(req, res) =>{
    ...
    const order = await Order.findById(req.params.id)
        ...
        .populate({
            ...
            populate: {
                path: 'clothing',
                select: 'name price description style category image'
            }
        }).lean().exec()
...
const show = async(req, res) =>{
    ...
    .populate({
        ...
        populate: {
            path: 'clothing',
            select: 'name price description style category image color'
        }
    }).lean().exec()
...
const create = async(req, res) =>{
    ...
    const newItems = []
    items.forEach(item => {
        price = price + parseInt(item.clothing.price)
        newItems.push({
            clothing: item.clothing._id,
            price: parseInt(item.clothing.price),
            color: item.color,
        })
    })

    //create and store new order
    const order = await Order.create({ orderid, customer, items: newItems, orderdate, price })
...
const updateStatus = async(req, res) =>{
    ...
    //confirm data
    if(!id || !status) return res.status(400).json({ message: 'All Fields are required', data: [] }) 
    ...
    res.json({ message: `Order's records have been ${status} successfully`, data: order })
}
